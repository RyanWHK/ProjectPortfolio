...
select 
  location, 
  population,max(total_cases) as highest_infection_count, 
  max((100*total_cases::float/population::float)) as population_infection_percentage 
from "CovidDeaths"
...
select 
  ...
  (100*sum(cast(new_deaths as float))/sum(cast(new_cases as int))) as DeathPercentage
from "CovidDeaths"
...
select 
  *, 
  (rolling_vaccinated::float / population::bigint)*100 as vac_vs_pop
from PopvsVac;
